...
import { BoardSelection } from "@/interfaces/interfaces";
    ...
    <div className={`boardSelectionArea ${toggle ? "" : "hide"}`}>
      <div className="boardSelectionWrapper">
        ...
        <div className="boardsWrapper">
          <ul className="boards">
            {boards.map((board) => (
              <li
                key={board.id}
                className={`board ${board.id === index && "active"}`}>
                <svg
                  className="eye"
                  width="16"
                  height="16"
                  ...
                  <path
                    ...
                    fill={board.id === index ? "#fff" : "#828FA3"}
                  />
                </svg>
                <button
                  onClick={() => {
        ...
        <div className="boardPanelOptions">
          ...
          <div className={`hideSideBarWrapper ${toggle ? "" : "toggle"}`}>
            ...
            <button onClick={() => setBoard(!toggle)}>hide sidebar</button>
          </div>
        </div>
      </div>
    </div>
